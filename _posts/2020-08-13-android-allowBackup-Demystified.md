...
2) Specify what you want to include or exclude from the backup using the syntax below:
    ...
    1) If a type is specified in both the include and the exclude tags, the item specified in the exclude tag takes precedence.

    2)You can add a new <include> or <exclude> tag for every new resource you want to specify.

    3)The path tag specifies the path to a resource you want to include or exclude. For example: <exclude domain="database" path="my_db.db"/>
